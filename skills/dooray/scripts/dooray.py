...
def build_parser() -> argparse.ArgumentParser:
    ...
    e = c_sub.add_parser("event-create", help="Create event; requires write policy and --apply")
    e.add_argument("calendar_id")
...
def run(args: argparse.Namespace, client: DoorayClient) -> object:
    ...
    if args.domain == "calendar":
        ...
        if args.action == "event-create":
            ...
            return create_event(args.calendar_id, subject=args.subject,
                                body_markdown=args.body, started_at=args.started_at,
                                ended_at=args.ended_at, whole_day=args.whole_day,
                                timezone=args.timezone, who_organization_member_ids=members,
